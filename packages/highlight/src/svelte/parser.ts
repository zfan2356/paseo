// Generated by lezer-generator from @replit/codemirror-lang-svelte@6.0.0
// Vendored to remove @codemirror/view and @codemirror/state DOM dependencies.
// Only pure Lezer primitives are used here.
...
import { svelteHighlighting } from "./highlight.js";

const spec_BlockPrefix: Record<string, number> = { "#": 41, ":": 51, "/": 59, "@": 109 };
const spec_BlockType = new Map<string, number>([
  ["if", 44],
  ["else", 52],
  ["each", 64],
  ["await", 82],
...
const spec_identifier = new Map<string, number>([
  ["if", 54],
  ["as", 66],
  ["then", 84],
  ["catch", 86],
]);
const spec_AttributeName: Record<string, number> = {
  on: 313,
  bind: 317,
  let: 319,
  class: 321,
...
const spec_TagName: Record<string, number> = { svelte: 243 };

export const parser = /*@__PURE__*/ LRParser.deserialize({
  version: 14,
  states:
  ...
  specialized: [
    { term: 19, get: (value: string) => spec_BlockPrefix[value] ?? -1 },
    { term: 21, get: (value: string) => spec_BlockType.get(value) ?? -1 },
    { term: 153, get: (value: string) => spec_identifier.get(value) ?? -1 },
    { term: 77, get: (value: string) => spec_AttributeName[value] ?? -1 },
    { term: 69, get: (value: string) => spec_TagName[value] ?? -1 },
  ],
  tokenPrec: 1571,
});
