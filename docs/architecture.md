...
>
> **Host-window link opens.** Chat, terminal, and other host-renderer http(s) navigations, including `window.open`, leave the Paseo window. On macOS they open in Google Chrome, then fall back to the system default browser. They must not become Electron windows or replace the host renderer. Guest browser tabs keep the window-open policy above.
>
